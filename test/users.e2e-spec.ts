...
describe('AppController (e2e)', () => {
  ...
  describe('/api/users/login (POST)', () => {
    ...
    it('should succeed if correct data', async () => {
      const registerResponse = await request(app.getHttpServer())
        ...
        .send({ email: 'test@test.com', password: 'test1234', passwordConfirm: 'test1234' })
        .expect(201)
      const loginResponse = await request(app.getHttpServer())
        .post('/api/users/login')
        .send({ email: 'test@test.com', password: 'test1234' })
        .expect(200)
      expect(loginResponse.body.user.email).toEqual(registerResponse.body.user.email)
  ...
  describe('/api/users/register (POST)', () => {
    ...
    it('should fail if incorrect email', async () => {
      await request(app.getHttpServer())
        .post('/api/users/register')
        .send({ email: 'test', password: 'test1234', passwordConfirm: 'test1234' })
        .expect(400)
    })
    ...
    it('should fail if password missmatch', async () => {
      await request(app.getHttpServer())
        .post('/api/users/register')
        .send({ email: 'test@gmail.com', password: 'test1234', passwordConfirm: 'incorrect' })
        .expect(400)
    })
    ...
    it('should fail if user exists', async () => {
      await request(app.getHttpServer())
        .post('/api/users/register')
        .send({ email: 'test1@gmail.com', password: 'test1234', passwordConfirm: 'test1234' })
        .expect(201)
    ...
    it('should succeed if correct data', async () => {
      await request(app.getHttpServer())
        .post('/api/users/register')
        .send({ email: 'test2@gmail.com', password: 'test1234', passwordConfirm: 'test1234' })
        .expect(201)
    })
  })
  ...
  describe('/api/users/me (get)', () => {
    ...
    it('should succeed if correct token', async () => {
      const registerResponse = await request(app.getHttpServer())
        .post('/api/users/register')
        .send({ email: 'test1234@gmail.com', password: 'test1234', passwordConfirm: 'test1234' })
        .expect(201)
